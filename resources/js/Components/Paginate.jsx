...
export default function Paginate({ contents }) {
    ...
    return (
        <div className={`grid grid-cols-3 md:flex md:justify-end`}>
            ...
            {links.length > 3 ? contents.current_page > 3 ? <PrimaryButton className={`m-2 w-fit`} disabled={true}>{`...`}</PrimaryButton> : <></> : <></>}
            {numberLinks.slice(contents.current_page === 1 ? 0 : contents.current_page - 2, contents.current_page + (paginate_show - 2)).map((item, index) => {
                return (
                    <Link href={item.url}>
